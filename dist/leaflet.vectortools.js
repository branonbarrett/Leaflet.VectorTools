...
;L.Toolbar = L.Class.extend({
	...
	_initModeHandler: function (handler, container, buttonIndex, classNamePrefix, buttonTitle) {
		var type = handler.type;
		var suffix = handler.classNameSuffix || type;
		...
		this._modes[type].button = this._createButton({
			title: buttonTitle,
			className: classNamePrefix + '-' + suffix,
			container: container,
...
L.Draw.Feature = L.Handler.extend({
	...
	_fireCreatedEvent: function (layer) {
		var type = this.type.split('_')[0];
		// if (this.type.indexOf('_') > -1) {
		// 	type = this.type.substring(this.type.indexOf('_'));
		// } else {
		// 	type = this.type;
		// }
		if (this.options.onCreatedEvent) {
			this._map.fire(this.options.onCreatedEvent, { layer: layer, layerType: type });
		} else {
			this._map.fire('draw:created', { layer: layer, layerType: type });
		}
	},
...
L.Draw.SimpleShape = L.Draw.Feature.extend({
	...
	initialize: function (map, options) {
		...
		this.type = L.Draw.Polyline.TYPE + this._uid();
		this.classNameSuffix = L.Draw.Polyline.TYPE;
...
;L.Draw.Polygon = L.Draw.Polyline.extend({
	...
	initialize: function (map, options) {
		...
		// Save the type so super can fire, need to do this as cannot do this.TYPE :(
		this.type = L.Draw.Polygon.TYPE + this._uid();
		this.classNameSuffix = L.Draw.Polygon.TYPE;
	},
...
;L.Control.Draw = L.Control.extend({
	...
	initialize: function (options) {
		...
		if (L.DrawToolbar && this.options.draw) {
			toolbar = new L.DrawToolbar(this.options);
